    ...
    <div className="space-y-6">
      ...
      {notifications.length === 0 ? (
        <div className="card flex flex-col items-center justify-center text-center p-12 border border-primary/20 bg-slate-900">
          ...
          <h3 className="text-slate-100 font-bold text-base">No notifications yet</h3>
          <p className="text-slate-500 text-xs mt-1.5">Alerts from dispatch will appear here.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {allRead && (
            <div className="card flex items-center gap-3 p-4 border border-primary/20">
              <CheckCircleIcon className="w-6 h-6 text-primary shrink-0" />
              <div>
                <p className="text-slate-100 font-bold text-sm">All caught up</p>
                <p className="text-slate-500 text-xs">Last checked: {lastCheckedTime || 'just now'} — history still shown below</p>
              </div>
            </div>
          )}
          {notifications.map((notif) => {
            ...
            const kind = (notif.priority === 'HIGH' || notif.category === 'ALERT')
              ? 'ALERT'
                ...
                <div className="flex items-start gap-4 flex-1">
                  <div
                    className={clsx(
                      ...
                      kind === 'ALERT' && (showAsUnread ? 'bg-red-50 text-red-500 border-red-200' : 'bg-status-critical/10 text-status-critical border-status-critical/20'),
                      kind === 'SUCCESS' && (showAsUnread ? 'bg-green-50 text-green-600 border-green-200' : 'bg-status-active/10 text-primary border-primary/20'),
                      kind === 'INFO' && (showAsUnread ? 'bg-blue-50 text-blue-500 border-blue-200' : 'bg-surface-light text-slate-400 border-surface-border')
                    )}
                  >
                    {kind === 'ALERT' && <ExclamationTriangleIcon className="w-5 h-5" />}
                    {kind === 'SUCCESS' && <CheckCircleIcon className="w-5 h-5" />}
                    {kind === 'INFO' && <InformationCircleIcon className="w-5 h-5" />}
                  </div>
